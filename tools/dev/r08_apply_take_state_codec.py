...
def replace_once(path: Path, old: str, new: str) -> None:
    ...
    if count != 1:
        raise SystemExit(f"{path}: expected block exactly once, found {count}")
...
replace_once(
    ...
    "// moving the library between Windows and macOS does not bake an obsolete\n"
    "// platform path into every song. end_frame_exclusive == 0 means full file.\n"
...
replace_once(
    ...
    "         binding.end_frame_exclusive <= binding.start_frame + 1U) ||\n"
...
replace_once(
    ...
    "  std::size_t take_binding_bytes = 4U + state.take_library_locator.size() + 2U;\n"
    "  for (const auto& binding : state.take_bindings)\n"
...
replace_once(
    ...
    "    for (const auto& binding : state.take_bindings) {\n"
    "      append_u16(result.bytes, static_cast<std::uint16_t>(binding.song_id.size()));\n"
    "      result.bytes.insert(result.bytes.end(), binding.song_id.begin(),\n"
    "                          binding.song_id.end());\n"
    "      append_u16(result.bytes, static_cast<std::uint16_t>(binding.file_name.size()));\n"
...
replace_once(
    ...
    "  // Take bindings must never allow directory traversal or ambiguous ranges.\n"
    "  {\n"
    "    auto invalid = state;\n"
...
# Replace legacy 1.0 setup so the new 1.3 tail is removed as well.
replace_once(
    tests,
...
# Replace legacy 1.1 block with an empty 1.3 take tail and remove both 1.2+1.3.
replace_once(
    tests,
...
# Add an explicit 1.2 migration test before the future-minor test.
replace_once(
    tests,
...
print("R08 take-state codec 1.3 patch applied")
